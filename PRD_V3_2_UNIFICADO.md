...
> ⚠️ **Nota (Faixa 1.6, Bloco 1, 2026-08-09) — uma segunda armadilha, distinta da acima: IC de TREINO e IC de OOF/realizado são populações diferentes, não duas medições do mesmo número.** A triagem (`screen_monotone_constraints`) roda sobre `train_side_df` — TODAS as barras de treino do fold, antes de qualquer seleção. Qualquer IC calculado sobre predições OOF/realizadas (`predictions.parquet`, `side_hat != 0`) é, por construção, medido só sobre a fração que já cruzou `tau` (~1,89% das barras, e de forma NADA uniforme por regime — medido: 0,3% em R1 a 4,3% em R2/R4, Faixa 1.6). Condicionar numa variável correlacionada com a saída do próprio modelo (a feature em questão é uma das 10 que alimentam essa saída) pode induzir correlação que não existe na população de treino completa — mesma família de viés de seleção/Berkson, não vazamento temporal. `E02f_funding_z_expanding` mostrou exatamente isso: IC de treino uniformemente negativo em R1-R4, 15/15 folds; IC pooled sobre OOF inverte de sinal em R1/R2. As duas medições estão corretas — respondem perguntas diferentes (o insumo da triagem vs. a população efetivamente negociada). Só a de TREINO configura o modelo — mesmo espírito da regra acima, população errada em vez de horizonte errado. Ver `experiments/faixa1_6_reconciliation.json::ic_reconciliation`.
