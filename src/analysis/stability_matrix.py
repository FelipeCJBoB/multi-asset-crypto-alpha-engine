"""ADR-008 Fase 5 — stability matrix (parcial, sem SHAP): cruza Fold ×
{IC, AUC/LogLoss, feature gain, decile returns} sobre o artefato de
walk-forward real (`src.models.walk_forward.run_walk_forward_for_combo`,
Fase 4, `experiments/alpha_walk_forward_{symbol}_{resolution_id}.json`)
— os 4 eixos já existem no artefato desde a Fase 5 parte 1
(`WalkForwardFoldMetrics.score_quality_by_side`/`decile_profile_by_side`/
`gain_by_column_by_side`), esta parte só CRUZA.
...
**Mede ESTABILIDADE, não só tabula.** Duas perguntas que a matriz
existe pra responder: (1) quão dispersa é cada métrica ENTRE folds
...
MESMA feature domina o gain em todo fold, ou o ranking muda
(`top_feature_frequency_by_side` — fração dos folds em que cada feature
foi #1 por gain bruto, ordenado decrescente). Um IC/AUC médio "bom" com
dispersão alta entre folds é um resultado instável, não confiável —
exatamente a leitura que médias sozinhas escondem."""
...
class StabilityRow:
    ...
    # `None` se o lado treinou mas gain bruto total foi 0 (booster nunca
    # dividiu em nenhuma feature) -- nunca inventa um "top" sem base.
...
class StabilityMatrixResult:
    ...
    # nenhum trade OOF nesse fold ainda aparece (gain existe, métricas
    # de trade ficam `NaN`, não ausente): treinar não exige ter
...
def _top_feature_by_gain(gain: dict[str, float]) -> tuple[str | None, float]:
    """`(feature, share)` — a feature de MAIOR gain bruto e sua fração do
    gain TOTAL do lado nesse fold. `(None, NaN)` se `gain` vazio ou soma
    zero (booster nunca dividiu em nenhuma feature) — nunca inventa um
    "top" sem base real."""
    if not gain:
    ...
    if total <= 0.0:
        return None, float("nan")
    top_feature = max(gain, key=lambda f: gain[f])
    return top_feature, gain[top_feature] / total


def _top_feature_frequency(rows: list[StabilityRow], side: str) -> dict[str, float]:
    """Fração dos folds (deste `side`) em que cada feature foi #1 por
    gain — ordenado decrescente. Dict vazio se nenhum fold deste lado
    teve um top-feature definido (gain sempre vazio/zero)."""
    side_rows = [r for r in rows if r.side == side and r.top_feature_by_gain is not None]
    if not side_rows:
        return {}
    counts: dict[str, int] = {}
    for r in side_rows:
        feature = r.top_feature_by_gain
...
def build_stability_matrix(
    ...
    for fr in fold_results:
        ...
        for side in _SIDES:
            ...
            top_feature, top_share = _top_feature_by_gain(gain)
            rows.append(
                StabilityRow(
                    fold_id=fr["fold_id"],
                    side=side,
                    ...
                    top_feature_by_gain=top_feature,
    ...
    top_feature_frequency_by_side = {side: _top_feature_frequency(rows, side) for side in _SIDES}
