"""Loader de `config/constants.yaml` — privado ao pacote `monitoring` (Regra
Zero, §16.10). Duplicado de `src/risk/_constants.py`/`src/data/_constants.py`
pelo mesmo motivo declarado em `_paths.py` deste pacote.

Nenhum literal numérico de limiar de alarme (ex. `dollar_bar_drift_alarm_
ratio_max`, AG-042 item 2) vive solto em código desta camada; tudo entra em
