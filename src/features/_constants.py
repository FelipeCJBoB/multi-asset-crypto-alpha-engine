"""Loader de `config/constants.yaml` — privado ao pacote `features` (Regra
Zero, §16.10). Duplicado de `src/data/_constants.py` pelo mesmo motivo
declarado em `_paths.py` deste pacote.

Nenhum literal numérico de janela de feature (EMA, ATR, RSI, z-score,
warmup) vive solto em código desta camada; tudo entra em `constants.yaml`
com proveniência declarada e é lido daqui.
