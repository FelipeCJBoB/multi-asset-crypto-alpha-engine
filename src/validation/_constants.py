"""Loader de `config/constants.yaml` — privado ao pacote `validation` (Regra
Zero, §16.10). Duplicado de `src/labels/_constants.py` pelo mesmo motivo
declarado em `_paths.py` deste pacote.

Nenhum literal numérico do CPCV (`cpcv_n_groups`, `cpcv_n_test_groups`,
`cpcv_embargo_bars`) vive solto em código desta camada; tudo entra em
`constants.yaml` com proveniência declarada e é lido daqui. `time_stop_bars`
(purge) e `maker_fee`/`taker_fee`/etc. já existem em `constants.yaml` desde
o Sprint 6 (labels) e são REUSADOS aqui via o mesmo `load_constant`, não
redeclarados.
