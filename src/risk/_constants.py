"""Loader de `config/constants.yaml` — privado ao pacote `risk` (Regra Zero,
§16.10). Duplicado de `src/regime/_constants.py` pelo mesmo motivo declarado
em `_paths.py` deste pacote.

Nenhum literal numérico de limiar do Risk Engine (sizing, os 18 controles, os
13 gatilhos do kill switch) vive solto em código desta camada; tudo entra em
...
processo; testes que precisam de outro valor passam override explícito em vez
de mexer no cache global.
"""
