...
| `features` (M1, `src/features/volatility.py`) | remedição dos 8/6 candidatos sob grade dollar (5 símbolos × R1/R2/R3) | ✅ pronto — Parkinson bate GK em 12/15 combinações (`AG-065`/`AG-074`), Manager decidiu Parkinson canônico 2026-08-17 | `AG-036` addendums `medicao_completa`/`decisao_manager_2026_08_17` |
...
| `data`+`config` (reprocessamento real, flip de `canonical_volatility_estimator.value`, retreino Alpha) | grade dollar+Parkinson construída em escala pros 5 símbolos, `value` flipado, Alpha Camada 1 retreinado | ⬜ **deliberadamente adiado 2026-08-17** — engenharia pronta (linha acima), corte real de produção agendado junto de outras mudanças já previstas no roadmap; Manager: "run canônico de produção agora seria desperdício de tempo" (não é bloqueio técnico) | `audit/n_lifetime.yaml` id 17 (`budget_override_manager`), `docs/refactor_parkinson_canonico.md` |
