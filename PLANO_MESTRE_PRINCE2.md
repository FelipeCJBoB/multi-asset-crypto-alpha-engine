...
| `11_META_MODEL` | V41-10 (Meta-Model) — **Grupo J desacoplado, movido para depois** (`§15.19`) | 🔴 **F0-F6b implementados, QUATRO medições convergentes reprovam** (2026-08-30 a 2026-09-01, `§15.30`/`§15.31`/`§15.38`-`§15.40`): `src/models/meta_dataset.py`, `src/models/meta.py` (orquestração F5.5 completa — `run_meta_fold`/`run_all_meta_folds`/`run_meta_sprint`), `src/models/meta_ablation.py` (F6 — A0/A1/A2-nulo/A3), `src/models/meta_walk_forward.py` (F6b — split causal único), `src/validation/cpcv.py` (`AG-151` fechado), `src/analysis/meta_fp_inventory.py` (P0-P3 completos). `P0`/`P1` fechados 2026-08-31 (9/10 calibrado, `AG-402` não bloqueante). **`P3`/Gate E0 REAL rodou 2026-08-31 contra os 5 combos de produção — 0 de 5 passam** (`AG-404`, vermelho). **F6 (ablação real) rodou 2026-09-01 — 0 de 5 SÍMBOLOS passam o critério primário** (`AG-409`, vermelho): 2 combos (SOLUSDT/R3, XRPUSDT/R3) sem nenhum fold capaz de ajustar modelo, Jaccard(A1,A3)=1,000 exato na maioria (Meta ≈ reparametrização de `tau`). **F6b (walk-forward ancorado, `§4.4`) rodou 2026-09-01 — 0 de 5, ainda mais decisivo** (`AG-413`, vermelho): os 5 combos caem em `INSUFFICIENT_SAMPLE`, o Meta nunca chega a ajustar modelo sob a estrutura causal real, nem `BTCUSDT/R2`. Quatro achados convergem por metodologia totalmente diferente: Alpha "0/20" (`AG-391`), Gate E0 "0/5" (`AG-404`), F6 "0/5 símbolos" (`AG-409`), F6b "0/5, sem amostra pra tentar" (`AG-413`). Consequência pré-declarada do design doc ("Meta sai do roadmap") NÃO aplicada — decisão explícita do Manager, 2026-09-01: "não é impeditivo agora, finalize a construção mesmo assim, eu investigo isso depois". F7/F8 seguem | `docs/meta_model_design_doc_2026-08-22.md`, `§15.19`, `§15.35`-`§15.40`, `AG-404`, `AG-409`, `AG-413` |
