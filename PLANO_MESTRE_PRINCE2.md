...
| `features` (M1, `src/features/volatility.py`) | remedição dos 8 estimadores sob grade dollar | ⬜ bloqueado — precisa dollar bar real construída em escala primeiro | `AG-036` |
| `data` (reprocessamento real, 61GB `aggTrades`) | grade dollar construída em escala, todo o pipeline rerodado | ⬜ não iniciado — mecanismo de memória corrigido (linha `bars.py` acima) e runner de validação provado numa janela pequena; reprocessamento em escala completa (histórico inteiro, 5 símbolos) ainda não tentado | `docs/refactor_dollar_bar_canonico.md` §5.3 |
