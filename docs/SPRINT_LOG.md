...
**Fechamento mesmo dia — full Optuna concluído, hiperparâmetros novos promovidos, primeira célula passa um gate com significância real.** Campanha completou em ~2h (`experiments/t0_cutoff_control_campaign_results_rate010_20260903.json`), 10/10 studies. `best_value` de screening caiu bastante em quase toda célula vs. a campanha anterior (0,0284) — `SOLUSDT/R2/C0` chegou a NEGATIVO (-0,1029, era +0,3095) — consistente com o achado da divergência: taxa mais alta dilui a população de trade admitida. Promovido (`run_stamp=20260903T192150Z`, `scripts.promote_control_campaign_to_production`). Run Canônico + walk-forward campaign + predictions regenerados sob o hiperparâmetro novo, 0 falhas.

**Marco real: `XRPUSDT/R3/Camada1` passa o gate Alpha** (edge=+22,75bps, p=0,030 &lt; 0,05) — primeira célula em toda a investigação (3 rodadas nesta sessão) a vencer qualquer um dos 3 gates com significância estatística real, não só "perto". Não sobrevive ao composto porque o gate Model reprova nos 2 lados dessa mesma célula (AUC 0,517/0,416). Gate Data: 7/10 células passam agora (era 6/10). Run Canônico: os 5 combos voltaram a Sharpe positivo (2 eram negativos na rodada anterior), mas `BTCUSDT/R2` (0,196) e `SOLUSDT/R2` (0,036) muito perto de zero — o Optuna achou o melhor ponto dentro de uma busca mais difícil, não recuperou o nível da rodada 1. Fase 4: 6/10 células com edge positivo agora (era 2/10).

Artefato "ADR-007" atualizado de novo com os números da rodada 3. Briefing técnico-financeiro de auditoria externa publicado, cobrindo a cadeia de código de produção + matemática financeira completa + o achado desta investigação (`https://claude.ai/code/artifact/9b13590c-bf34-4828-9290-1cd2a772ea67`). Detalhe completo: `audit/architecture_gaps_log.yaml::AG-428`. Investigação da deriva temporal permanece **aberta** — causa exata não isolada quantitativamente, só inferida por eliminação. Fases 5/7/8 + as 2 auditorias continuam não refeitas.

**Rodada 4 mesmo dia (2026-09-03/04) — auditoria externa adversarial aplicada ponta a ponta; o resultado é NEGATIVO e é o achado.** Manager autorizou os 13 itens do retorno da auditoria + execução de `uv`/`.py`. Dois foram **rejeitados como especificados** e substituídos pela versão correta, após leitura do código: (a) "gap-through no TP, simétrico ao SL" inflaria o edge em vez de reduzi-lo — TP é `LIMIT`/`GTC`, executa no preço de repouso, gap-through é irrelevante pra ordem passiva; o que sobrou do achado virou cobrança de seleção adversa (`AG-432`); (b) "aplicar `floor_to_step` no `backtest_lite`" é inaplicável — o módulo opera só em espaço de retorno, sem `qty`/`notional`/`mark_price`; virou camada nova de viabilidade de capital (`AG-435`). Ver `audit/architecture_gaps_log.yaml` (`AG-429`..`AG-437`).

`AG-432` mudou `ret_net` (seleção adversa cobrada nas pernas maker: 3,0bps em TP, 1,5 em SL/TIME) e portanto o `config_hash` — o guardrail B15 disparou corretamente e forçou rebuild dos 15 `labels.parquet` (backup em `data/labels_pre_ag432_relabel`). Retreino completo em cima: Run Canônico + walk-forward campaign + predictions, 0 falhas.

**Resultado: 0/10 no composto e o gate Alpha caiu de 1/10 para 0/10.** `XRPUSDT/R3/C1`, a única célula que já tinha passado um gate com significância, foi de +22,75bps para +12,29bps e perdeu. Run Canônico (CPCV): os 5 combos viraram Sharpe NEGATIVO (eram 0,15/-0,10/0,50/1,02/0,86 → -2,75/-1,03/-0,32/-0,44/-0,45), com 46% mais trades (47.866→69.716). Gate Data segue 7/10.

**Atribuição honesta, medida** (a cobrança vive em `src/labels/triple_barrier.py`, `AG-432`): a deterioração de edge/trade foi de -5,5 a -10,6bps por combo, enquanto a cobrança de seleção adversa vale -1,5 a -3,0bps. O custo explica só ~25-40%; o resto vem dos modelos re-treinados sob `sample_weight` alterado (`|ret_net|` mudou). A leitura defensável não é "a cobrança matou o edge" — é que **o edge aparente não sobrevive a uma perturbação de 1,5-3,0bps no modelo de custo**, o que é o próprio diagnóstico. Um teste isolante (rodar o código novo com `adverse_selection_bps=0`) custa 1 rebuild + 1 retreino e está declarado como pendente, não feito.

**`AG-437`, achado próprio a partir do rastro do revisor:** "a taxa realizada fica sistematicamente em ~65-70% da nominal" — conclusão central do `AG-428`, commitada em `config/constants.yaml`, no ledger e no briefing externo — é **artefato de pooling**. Por célula a razão vai de 0,18x a 1,05x (spread 5,9x); por trimestre, de 0,475 a 1,476 (amplitude 3,11x) com Spearman(tempo, razão)=+0,336, ou seja **sem tendência monotônica**. Dois eixos independentes dizem o mesmo: o problema é DISPERSÃO em torno do alvo, não viés pra baixo. Pior, o dado por célula já estava no relatório que `scripts/ag428_signal_rate_decomposition.py` emitiu na rodada 3 — reportei só o agregado. É a mesma falha de processo do `AG-429`, reincidente no mesmo dia. Regra que sai disto: **todo número agregado publicado vem acompanhado da dispersão que ele resume.**

**Hipótese NOFILL confirmada por medição (`AG-436`)**, fechando a especulação que o revisor deixou em aberto: em `XRPUSDT/R3/C1`, remover as barras NOFILL da pool out-of-fit desloca a taxa mediana de 1,76x para 0,64x — fator 2,76x atribuível a uma variável só. É o mecanismo dominante do overshoot que fez o `AG-428` descartar `TOTAL_COMMON_OOF`. Não promovível ainda: a correção CRUZA o alvo em vez de acertar.

**`AG-435` — primeira chamada real do código de risco de produção sobre trades reais** (fecha o achado B1: o teto de 5,88%/mês tinha sido replicado à mão, nunca commitado). Três resultados: (1) quantização NÃO inviabiliza nada — `frac_executavel`=1,00 nas 10 células a R$1.000, e o erro mediano (2,3% em BTC, 0,01% em XRP) está muito abaixo dos 16,2% que o PRD documenta; (2) `control_13_orcamento_fees` REPROVA `BTCUSDT/R2` nas 2 camadas — 4,88% e 6,46% do equity/mês, e o C0 estoura o teto autorizado; (3) o edge **ponderado pelo nocional** diverge muito do nominal e sempre pra pior nas células positivas (`XRPUSDT/R3/C1`: +8,65 → +0,43; C0: +5,68 → -8,45) — o edge positivo se concentra nos trades de nocional pequeno, então o gate Alpha, que pesa cada trade igualmente, superestima o que R$1.000 renderiam.

Itens 7 e 11 também fechados: Q10-Q1 revalidado sob o regime atual continua sem sinal (troca de sinal entre lados/camadas, consistência de fold ~50%, o próprio critério de ruído do script); e o gate Model migrou pra população completa (`AG-433`) — medição de validação mostrou que a fonte antiga cobria 148/198 fold-lados contra 198/198 da nova, ou seja o `n_folds` do teste-t vinha subestimado em 25% dos casos por ausência silenciosa. Pendente declarado: `AG-434` (sweep de janela com ≥5 seeds e pontos de 365/540 dias) tem a grade corrigida no código e execução não feita — é a medição mais cara da lista.

**Campanha multi-seed + causa-raiz do `AG-428` FECHADA (2026-09-04).** Manager pediu duas coisas: investigar a fundo por que `target_signal_rate=0,10` nunca é entregue, e rodar o multi-seed (`scripts/run_walk_forward_campaign.py --seed N`) pra descobrir se existe edge em alguma célula.

**Causa-raiz (`AG-438`): discretização do calibrador.** Não era deriva temporal, nem exclusividade mútua, nem pool pequena, nem "deriva sistemática" — as três hipóteses anteriores descreviam sintomas. `tau` é o quantil `1-target_signal_rate` da pool calibrada, e isso só entrega a taxa pedida se a pool for aproximadamente contínua. A isotônica colapsa a saída em poucos níveis (o PAV funde blocos por construção quando o score não separa), então o quantil encosta na BORDA DE UM PLATÔ; e `decide_side` usa `p > tau` estrito, descartando de uma vez a massa presa exatamente em tau. Medido em 858 fold×lado (5 seeds): mediana de 9 níveis distintos, mínimo 1, 96,3% com ≤20; massa em tau com mediana 25,6%, p90 91,5% e máximo 100%. **Em 82,6% dos fold×lado, 0,10 ±20% é inatingível sob QUALQUER escolha de tau** — não é erro de calibração, é impossibilidade aritmética. Caso que fecha: `SOLUSDT/R3/C0` fold 1 tem 2 níveis em 1.113 barras, 98,47% em `p=0,526152` que É o tau; taxas atingíveis {0,0153; 1,0}. Instrumentado (`tau_pool_discretization`, núcleo puro + 4 testes) e persistido por fold/lado no artefato, ao lado do `tau_pool_n` do `AG-430`.

Isso reinterpreta 3 investigações: o overshoot de 2,3-3,0× do `TOTAL_COMMON_OOF` (`AG-428`) e o deslocamento de 2,76× ao remover NOFILL (`AG-436`) são ambos relocações da borda do platô, não correções de calibração. E torna `target_signal_rate` efetivamente **inerte** — subir 3,5× mudou o realizado muito menos que proporcionalmente porque o quantil encosta no mesmo platô. Registrado em `config/constants.yaml` que varrer essa constante no regime atual mede ruído de platô, não sensibilidade econômica.

**É sintoma, não doença** (a cadeia inteira vive em `src/models/alpha.py`). O PAV só funde tudo quando o score não discrimina, e a AUC de população completa (`AG-433`) está entre 0,4985 e 0,5107 nas 10 células — a ±0,011 do acaso puro, com n na casa das dezenas de milhares. Cadeia: sem poder discriminativo → isotônica colapsa → quantil encosta em platô → taxa arbitrária. Consertar tau sem consertar a AUC troca um número arbitrário por outro.
